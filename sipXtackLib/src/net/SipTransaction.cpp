    ...
    if(relationship == MESSAGE_DUPLICATE)
    {
        ...
        if(!(incomingMessage.isResponse()))
        {
            ...
            if(response)
            {
                ...
                {
                case OsSocket::UDP:
                    userAgent.sendUdp(response, sendAddress.data(), sendPort, NULL);
                    break;

                case OsSocket::TCP:
                    userAgent.sendTcp(response, sendAddress.data(), sendPort, NULL);
                    break;

#               ifdef SIP_TLS
                case OsSocket::SSL_SOCKET:
                    userAgent.sendTls(response, sendAddress.data(), sendPort, NULL);
                    break;
#               endif
        ...
        else
        {
                ...
                )
            {
                ...
                {
                case OsSocket::UDP:
                    userAgent.sendUdp(mpAck, sendAddress.data(), sendPort, NULL);
                    break;
                case OsSocket::TCP:
                    userAgent.sendTcp(mpAck, sendAddress.data(), sendPort, NULL);
                    break;
#               ifdef SIP_TLS
                case OsSocket::SSL_SOCKET:
                   userAgent.sendTls(mpAck, sendAddress.data(), sendPort, NULL);
                   break;
#               endif
                default:
                   Os::Logger::instance().log(FAC_SIP, PRI_CRIT,
